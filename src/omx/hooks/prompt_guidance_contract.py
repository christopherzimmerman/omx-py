...
Port of src/hooks/prompt-guidance-contract.ts (1:1 sync conversion).

Defines `GuidanceSurfaceContract` and the canonical contract tuples that
declare which patterns must appear in each guidance surface file (the
AGENTS.md root template, role prompts, scenario prompts, skill SKILL.md
files, etc.). Each contract is a static declaration; reading the surface
files and asserting compliance is the responsibility of callers (e.g. the
catalog/contract-verification surface).
"""
...
class GuidanceSurfaceContract:
    ...
    Attributes:
        ...
        path: Repo-relative path to the guidance file (forward-slash form).
        required_patterns: Regex patterns that must appear somewhere in the
            file contents. All patterns are compiled case-insensitively.
    """

    id: str
    path: str
    required_patterns: tuple[re.Pattern[str], ...] = field(default_factory=tuple)


def _rx(pattern: str) -> re.Pattern[str]:
    """Compile a case-insensitive regex (mirrors TS `rx` helper)."""
...
_ROOT_TEMPLATE_PATTERNS: tuple[re.Pattern[str], ...] = (
...
_CORE_ROLE_PATTERNS: dict[str, tuple[re.Pattern[str], ...]] = {
    "executor": (
...
_WAVE_TWO_PATTERNS: tuple[re.Pattern[str], ...] = (
...
WAVE_TWO_CONTRACTS: tuple[GuidanceSurfaceContract, ...] = tuple(
    GuidanceSurfaceContract(
        id=name,
        path=f"prompts/{name}.md",
        required_patterns=_WAVE_TWO_PATTERNS,
    )
    for name in _WAVE_TWO_NAMES
)

_CATALOG_NAMES: tuple[str, ...] = (
    "analyst",
    "api-reviewer",
    "build-fixer",
    "dependency-expert",
...
CATALOG_CONTRACTS: tuple[GuidanceSurfaceContract, ...] = tuple(
    GuidanceSurfaceContract(
        id=name,
        path=f"prompts/{name}.md",
        required_patterns=_CATALOG_PATTERNS,
    )
...
_SKILL_CONTRACT_NAMES: tuple[str, ...] = (
    "analyze",
    "autopilot",
    "build-fix",
    "code-review",
